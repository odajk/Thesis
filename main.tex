...
\begin{algorithm}[H]\label{algo:MH}
    ...
    \begin{algorithmic}[1] % The number tells where the line numbering should start\
        ...
        \If{ $\frac{\textsc{JointPosterior} \left(\theta'; \{z_n\}_{n = 1}^N\right)g\left(\theta_{i-1}\mid \theta'\right)}{\textsc{JointPosterior} \left(\theta_{i-1}; \{z_n\}_{n = 1}^N\right)g\left(\theta'\mid \theta_{i-1}\right)} > u$}
        \State $\theta_i = \theta'$
        \Else 
...
\todo{Noen ting jeg ikke har fått rettet opp i siden sist}
...
\end{split}
\end{equation*}{}
\todo{Bruker samme subscript for bestemt indeks som for generelle indekser} This can be simplified further by using that each $z_n$ is only dependent on the $n$'th data point $x_n$ and that the $x_n$'s are conditionally independent given $\theta$. The result is the following joint probability 
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1] % The number tells where the line numbering should start
        ...
        \State $u\sim \textsc{Uniform}\left(0,1\right)$
        \If{ $\frac{\textsc{JointPosterior} \left(\theta'; \{z_n\}_{n = 1}^N\right)}{\textsc{JointPosterior} \left(\theta_{i-1}; \{z_n\}_{n = 1}^N\right)} > u$}
        \State $\theta_i = \theta'$
        \Else 
        \State $\theta_i = \theta_{i-1}$
         \EndIf
...
\todo{Denne seksjonen er ikke så aktuell akkurat nå}
...
\begin{algorithm}[H]
    ...
    \Function{ConfidenceSampler}{$p\left(x\mid \theta \right), p\left(\theta\right), q\left(\theta'\mid \theta\right), \theta_0, N_{iter}, \chi, \left(\delta_t\right), C_{\theta, \theta'}$}
       ...
       \State $\Lambda^{\star} = \frac{1}{b} \left(t\Lambda^{\star} + \sum_{i = t+1}^b \log \left[\frac{p\left(x_i^{\star}\mid\theta'\right)}{p\left(x_i^{\star}\mid \theta\right)}\right]\right)$
       \State $t = b$
       ...
       \EndIf
       \EndFor
       \State \Return $\left(\theta_k\right)_{k = 1, \ldots, N_{iter}}$
       \EndFunction
    \end{algorithmic}
\end{algorithm}
